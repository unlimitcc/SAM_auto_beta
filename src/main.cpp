...
void generate_TRACE_property(string name){
    ...
    while(!property_infile.eof()){
        ...
        for(int i=0; i<var_exp.size()-1; i++){
            while(line.find(var_exp[i])!=string::npos){                                     //需匹配最长字符串,例如有命题count以及count+1，性质中出现count+1时可能出现错误
...
void generate_execu_log(){
    ...
    string filename = get_filename("Z:\\computer science\\VScode\\vscode_project\\IP_verify\\IP_GyroChoose\\Implement\\GyroChoose.c");//文件名与IP名称，内部结构体名称一致
...
void generate_GDB_script(){
    ...
    for(int i=0; i<var.size(); i++){
        if(var_type[var[i]].length()>0) GDB_script<<"\tdisplay "<<filename<<"1."<<var[i]<<endl;//在IP初始结构体中出现的变量使用display关键字即可，未在IP中定义，但在命题中出现的需要单独打印
    }
    while(!init_code.eof()){
        string code;
